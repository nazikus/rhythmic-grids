...
QUnit.test("Test against reference grid (obtained manually)", function (assert) {
...
QUnit.test("Total combinations test: computed and obtained number of configurations must be equal",
    function (assert) {
...
QUnit.test("Integer test: block width and height must be integers",
    function (assert) {
        ...
        for (var g = 0; g < valid_gc.length; g++)
        {
            ...
            assert.ok(rg.W % 1 == 0 && rg.H % 1 == 0, 
                'Grid dimensions are integer: ' + [rg.W,rg.H].join('x'));

            for (var b = 0; b < rg.blocks.length; b++){
                assert.ok(rg.blocks[b][1] % 1 == 0 && rg.blocks[b][2] % 1 ==0,
                    'Block #' + rg.blocks[b][2] + ' dimensions are integer: '
                     + rg.blocks[b].slice(0,2).join('x'));
            }
        }
});


QUnit.test("Grid width test: grid width must be less-equal then canvas width", 
    function (assert) {
        for (var g = 0; g < valid_gc.length; g++)
        {
            assert.ok(true, '>>>>> Grid width ' + valid_gc[g].rhythmicGrid.W + 'px, '
                            ...
                            + 'columns ' + valid_gc[g].columnsNum + ', '
                            + 'gutter ' + valid_gc[g].gutter.W+ 'px <<<<<');
            assert.ok(valid_gc[g].rhythmicGrid.W <= valid_gc[g].maxCanvasWidth, 
                'Grid width is less then or equal to canvas width');
        }
});


QUnit.test("Columns test: computed number of columns must be equal to input number of columns", 
    function (assert) {
        var gw = 0, cols = 0;
        for (var g = 0; g < valid_gc.length; g++)
        {
            assert.ok(true, '>>>>> max canvas width ' + valid_gc[g].maxCanvasWidth + 'px, '
                            + 'ratio ' + valid_gc[g].ratio.str + ', '
                            + 'baseline ' + valid_gc[g].baseline + 'px, '
                            + 'INPUT COLUMNS ' + valid_gc[g].columnsNum + ', '
                            + 'gutter ' + valid_gc[g].gutter.W+ 'px <<<<<');
            gw = valid_gc[g].gutter.W;
            cols = (valid_gc[g].rhythmicGrid.W+gw) / (valid_gc[g].rhythmicGrid.uBlock.W+gw);
            assert.strictEqual(cols, valid_gc[g].columnsNum, 
                'Computed column quanitity equals to input: ' + cols);
        }
});


QUnit.test("Baseline factor test: block height must be divisible by baseline",
    function (assert) {
        var rg = null; // rhythmic grid
        for (var g = 0; g < valid_gc.length; g++)
        {
            ...
            for (var b = 0; b < rg.blocks.length; b++){
                assert.ok(rg.blocks[b][1] % valid_gc[g].baseline == 0,
                    'Block #' + rg.blocks[b][2] + ' [' + rg.blocks[b].slice(0,2).join('x') +
                    '] is a factor of baseline ' + valid_gc[g].baseline + 'px');
            }
        }
});
